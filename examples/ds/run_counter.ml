...
let n_fiber_incr t n () =
  ...
  Picos_std_structured.Run.all thunks
...
let () =
  ...
  match sched with
  | "threaded" -> Picos_mux_thread.run main
  | "fifos" -> Picos_mux_fifo.run main
  | "randos" ->
      ...
      let spawn_ndoms_with_nthreads nd nt =
        for _ = 1 to nd do
          Domain.spawn (fun () ->
              for _ = 1 to nt do
                Thread.create Picos_mux_random.runner_on_this_thread context
                |> ignore
              done;
      ...
      Picos_mux_random.run ~context main
