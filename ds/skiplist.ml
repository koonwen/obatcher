  ...
  module Batched = struct
    module S = struct
      ...
      let par_insert t (pool : Task.pool) (elems : V.t array) =
        ...
        Task.parallel_for pool ~start:0 ~finish:(num_elems-1)
      ...
      let par_search t (pool : Task.pool) (elems : V.t array) : bool array =
        let result_arr = Array.make (Array.length elems) false in
        Task.parallel_for pool ~start:0 ~finish:((Array.length elems) - 1)
          ~body:(fun i -> result_arr.(i) <- Sequential.mem t elems.(i));
        result_arr

      let par_size t (pool : Task.pool) (elems : int array) : int array =
        let size = Sequential.size t in
        Task.parallel_for pool ~start:0 ~finish:((Array.length elems) - 1)
          ~body:(fun i -> elems.(i) <- size);
        elems

      let run t (pool: Task.pool) (ops: wrapped_op array) : unit =
        let inserts: V.t list ref = ref [] in
        let searches: (V.t * (bool -> unit)) list ref = ref [] in
        let size = lazy (Sequential.size t) in
        Array.iter (function
            | Mk (Size, kont) -> Promise.resolve kont (Lazy.force size)
            | Mk (Member vl, kont) -> searches := (vl, Promise.resolve kont) :: !searches
            | Mk (Insert vl, kont) -> Promise.resolve kont (); inserts := vl :: !inserts
          ) ops;
        (* now, do all searches in parallel *)
        let searches = Array.of_list !searches in
        Task.parallel_for pool ~start:0 ~finish:(Array.length searches - 1)
          ~body:(fun i ->
    ...
    let insert t elem = B.apply t (S.Insert elem)
    let size t = B.apply t S.Size
    let mem t elem = B.apply t (S.Member elem)
