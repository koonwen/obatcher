...
(*       type wrapped_op = *)
(*         | Mk : ('elt, 'ret) op * 'a Picos.Computation.t -> wrapped_op *)
...
(*       let rec par_search_node ?(par_threshold = 6) ?(threshold = 64) node *)
(*           ~height ~keys ~range:(rstart, rstop) = *)
...
(*         if n <= 0 then () *)
(*         else if n = 1 then *)
(*           let k, comp = keys.(rstart) in *)
(*           comp *)
(*             (Option.map *)
(*                (fun (node, i) -> node.Sequential.values.!(i)) *)
(*                (Sequential.search_node node k)) *)
(*         else if rstop - rstart < par_threshold && height > 5 then *)
(*           Utils.parallel_for ~start:rstart ~finish:(rstop - 1) (fun i -> *)
(*               let k, comp = keys.(i) in *)
(*               comp *)
(*                 (Option.map *)
(*                    (fun (node, i) -> node.Sequential.values.!(i)) *)
(*                    (Sequential.search_node node k))) *)
(*         else if *)
(*           (rstop - rstart < threshold && height < 3) || node.Sequential.leaf *)
(*         then *)
...
(*               && V.compare (fst keys.(!last_sub_interval_end)) node.keys.!(i) *)
(*                  < 0 *)
(*             do *)
...
(*             (fun i -> *)
(*               par_search_node ~par_threshold ~threshold node.children.!(i) *)
(*                 ~keys ~height:(height - 1) ~range:sub_intervals.!(i)) *)

(*       let par_search ?par_threshold ?threshold (t : 'a t) keys = *)
(*         let threshold = *)
...
(*           let min_child_capacity = *)
(*             Utils.parallel_for_reduce ~start:0 *)
(*               ~finish:(Finite_vector.length sub_intervals - 1) *)
(*               ~body:(fun i -> *)
(*                 let start, stop = sub_intervals.!(i) in *)
...
(*         then *)
(*           ignore *)
(*             (par_insert_node ?threshold ~max_children:t.max_children t.root *)
(*                batch start stop) *)
(*         else if *)
(*           (2 * t.max_children) - 1 *)
...
(*             match elt with *)
(*             | Mk (Insert (key, vl), comp) -> *)
(*                 Computation.return comp (); *)
(*                 inserts := (key, vl) :: !inserts *)
(*             | Mk (Search key, comp) -> *)
(*                 searches := (key, Computation.return comp) :: !searches *)
(*             | Mk (Size, comp) -> Computation.return comp start_size) *)
(*           ops; *)
